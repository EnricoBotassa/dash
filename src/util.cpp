...
const char * const BITCOIN_CONF_FILENAME = "zalgocoin.conf";
const char * const BITCOIN_PID_FILENAME = "zalgocoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "zalgocoin" is a composite category enabling all Zalgocoin-related debug output
                if(ptrCategory->count(std::string("zalgocoin"))) {
...
#else
    const char* pszModule = "zalgocoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ZalgocoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ZalgocoinCore
    // Mac: ~/Library/Application Support/ZalgocoinCore
    // Unix: ~/.zalgocoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ZalgocoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ZalgocoinCore";
#else
    // Unix
    return pathRet / ".zalgocoincore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty zalgocoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override zalgocoin.conf
