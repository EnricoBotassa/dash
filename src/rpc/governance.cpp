...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in zalgocoin.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("zalgocoin.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Zalgocoin features */
    { "zalgocoin",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "zalgocoin",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "zalgocoin",               "gobject",                &gobject,                true,  {} },
    { "zalgocoin",               "voteraw",                &voteraw,                true,  {} },
